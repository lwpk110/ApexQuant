...
当前版本覆盖八个控制室目的地（总览、策略实验室、回测中心、模拟执行、账户与风险、数据中心、运行记录、设置）。总览、数据中心和运行记录已通过后端 API 读取 Yahoo Finance 公共行情及进程运行记录；真实账户和真实券商仍未接入，不发送真实委托。策略研究和模拟执行中的非行情示例数据仍属于产品演示域，后续按读模型逐页替换。
