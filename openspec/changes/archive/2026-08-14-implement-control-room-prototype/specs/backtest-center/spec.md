...
Starting a backtest SHALL create an identifiable run snapshot and SHALL not
send a broker order. The result summary SHALL present run ID, state, stage,
progress, elapsed time, leakage-check result, out-of-sample range, cost model,
and data version.

#### Scenario: Start a backtest
- **WHEN** the user starts a valid backtest
- **THEN** a new run identifier is shown with its configuration snapshot metadata.
