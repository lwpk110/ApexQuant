...
export function DataPage({ diagnostic }: { diagnostic?: DataDiagnostic }) { const [data, setData] = useState<CatalogResponse | null>(null); const [error, setError] = useState(""); const load = () => { getCatalog().then(value => { if (!value?.datasets) throw new Error("API 返回的数据目录不完整"); setData(value); }).catch(e => setError(e instanceof Error ? e.message : "数据目录不可用")); }; useEffect(() => { load(); }, []);
